...
TEST(LSMLeveled, PutGet) {
    ...
    std::shared_ptr<ILSM> lsm = MakeLeveledLsm(GranularLsmOptions{}, files_provider, sstable_factory);
...
TEST(LSMLeveled, MultipleFlushesLatestWins) {
    GranularLsmOptions options;
    options.memtable_bytes = 1'000;

    std::shared_ptr<ISSTableSerializer> sstable_factory = MakeSSTableFileFactory();
    auto files_provider = std::make_shared<TestLevelsProvider>();
    std::shared_ptr<ILSM> lsm = MakeLeveledLsm(options, files_provider, sstable_factory);
...
TEST(LSMLeveled, WriteAmplificationBounded) {
    GranularLsmOptions options;
...
TEST(LSMLeveled, SearchComplexityByKeyAge) {
    GranularLsmOptions options;
...
TEST(LSMLeveled, GetWithSequenceNumber) {
    std::shared_ptr<ISSTableSerializer> sstable_factory = MakeSSTableFileFactory();
    auto files_provider = std::make_shared<TestLevelsProvider>();
    std::shared_ptr<ILSM> lsm = MakeLeveledLsm(GranularLsmOptions{}, files_provider, sstable_factory);
...
TEST(LSMLeveled, ScanMultipleKeys) {
    std::shared_ptr<ISSTableSerializer> sstable_factory = MakeSSTableFileFactory();
    auto files_provider = std::make_shared<TestLevelsProvider>();
    std::shared_ptr<ILSM> lsm = MakeLeveledLsm(GranularLsmOptions{}, files_provider, sstable_factory);
...
TEST(LSMLeveled, ScanSkipsTombstones) {
    std::shared_ptr<ISSTableSerializer> sstable_factory = MakeSSTableFileFactory();
    auto files_provider = std::make_shared<TestLevelsProvider>();
    std::shared_ptr<ILSM> lsm = MakeLeveledLsm(GranularLsmOptions{}, files_provider, sstable_factory);
...
TEST(LSMLeveled, ScanDeduplicatesVersions) {
    std::shared_ptr<ISSTableSerializer> sstable_factory = MakeSSTableFileFactory();
    auto files_provider = std::make_shared<TestLevelsProvider>();
    std::shared_ptr<ILSM> lsm = MakeLeveledLsm(GranularLsmOptions{}, files_provider, sstable_factory);
...
TEST(LSMLeveled, ScanAcrossLevels) {
    std::shared_ptr<ISSTableSerializer> sstable_factory = MakeSSTableFileFactory();
    auto files_provider = std::make_shared<TestLevelsProvider>();
    std::shared_ptr<ILSM> lsm = MakeLeveledLsm(GranularLsmOptions{}, files_provider, sstable_factory);
...
TEST(LSMLeveled, ScanWithSequenceNumber) {
    std::shared_ptr<ISSTableSerializer> sstable_factory = MakeSSTableFileFactory();
    auto files_provider = std::make_shared<TestLevelsProvider>();
    std::shared_ptr<ILSM> lsm = MakeLeveledLsm(GranularLsmOptions{}, files_provider, sstable_factory);
...
TEST(LSMLeveled, CompactionIsGranular) {
    GranularLsmOptions options;
    options.memtable_bytes = 1024;
    options.max_sstable_size = 4096;
    options.bloom_filter_size = 1024;
