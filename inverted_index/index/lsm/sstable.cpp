...
class FileSSTableReader final : public ISSTableReader {
   public:
    ...
    GetKind Get(const UserKey& user_key, Value* out_value) const override {
        *out_value = {};
        InternalKey internal_key = {user_key, ValueType::kValue};
    ...
    class SSTableViewer {
       public:
        ...
        std::pair<InternalKey, Value> GetObject(size_t ind) const {
            ...
            auto temp_buf = file_->Read(file_->Size() - offsets.first + sizeof(uint64_t), offsets.first - offsets.second - sizeof(uint64_t));
            object.first.user_key = std::string(temp_buf.begin(), temp_buf.end());
   ...
   public:
    ...
    void Finish() override {
        ...
        for (auto& object : objects_) {
            mem += object.first.user_key.size() + object.second.getSizeInBytes() + sizeof(uint64_t);
        }
        ...
        for (size_t ind = 0; ind < object_count; ++ind) {
            ...
            std::vector<uint8_t> sequence_number(sizeof(uint64_t));
