...
class FilterImpl : public IFilter {
   public:
    ...
    FilterImpl(const std::vector<char>& data) {
    ...
    std::vector<char> Serialize() {
        std::vector<char> buffer(sizeof(size_t) + bitset_.size() + p_.size());
...
class FilterBuilderImpl : public IFilterBuilder {
   public:
    ...
    std::vector<char> Serialize() { return filter_.Serialize(); }
...
class FilterDeserializerImpl : public IFilterDeserializer {
   public:
    std::unique_ptr<IFilter> Deserialize(const std::vector<char>& data) const { return std::make_unique<FilterImpl>(data); }
