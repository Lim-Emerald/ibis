...
class IMerger : public IStream<std::pair<K, V>> {
   public:
    // Returns next element in sorted order, or nullopt if all streams exhausted
    std::optional<std::pair<K, V>> Next() override = 0;
...
class KWayMerger final : public IMerger<K, V, Compare> {
   public:
    explicit KWayMerger(std::vector<std::shared_ptr<IStream<std::pair<K, V>>>> sources, Compare comp) : sources_(std::move(sources)), comp_(comp) {
        // std::cout << "Merger Start\n";
    ...
    std::optional<std::pair<K, V>> Next() override {
        if (heap_.empty()) {
            return std::nullopt;
        }
   ...
   private:
    class BigCompare {
       public:
        bool operator()(const std::pair<std::pair<K, V>, size_t> f, const std::pair<std::pair<K, V>, size_t> s) const { return Compare()(f.first.first, s.first.first); }
    };

    std::multiset<std::pair<std::pair<K, V>, size_t>, BigCompare> heap_;
    std::vector<std::shared_ptr<IStream<std::pair<K, V>>>> sources_;
    Compare comp_;
};
...
template <typename K, typename V, typename Compare = std::less<K>>
std::shared_ptr<IMerger<K, V, Compare>> MakeMerger(std::vector<std::shared_ptr<IStream<std::pair<K, V>>>> sources, Compare comp = Compare()) {
    return std::make_shared<internal::KWayMerger<K, V, Compare>>(std::move(sources), comp);
}
