...
class ILSM {
   public:
    ...
    virtual std::shared_ptr<IStream<std::pair<UserKey, Value>>> Scan() const = 0;
...
std::unique_ptr<ILSM> MakeIndexLsm(const std::string& dir, const std::pair<UserKey, Value> example, const GranularLsmOptions& options);
