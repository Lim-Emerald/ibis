...
class ISSTableBuilder {
   public:
    ...
    virtual void Add(const UserKey& internal_key, const Value& value) = 0;
...
class ISSTableReader {
   public:
    ...
    virtual std::shared_ptr<IStream<std::pair<UserKey, Value>>> MakeScan() const = 0;
...
std::shared_ptr<ISSTableSerializer> MakeIndexSSTableFileFactory(std::pair<UserKey, Value> example);
